package constant;

public class TitleConstant {
    private TitleConstant() {}

    public static final String APPLICATION_TITLE = "Homebanking";
    public static final String ADD_CLIENT_FORM = "Formulario para agregar cliente";
    public static final String CLIENT_PRODUCT_FORM = "Gestionar los productos";
    public static final String PRODUCT_CARDS_FORM = "Tarjetas del producto";
    public static final String ADD_BALANCE_FORM = "Agrega dinero a la cuenta";
    public static final String TRANSFER_FORM = "Transferir dinero entre cuentas";

}
